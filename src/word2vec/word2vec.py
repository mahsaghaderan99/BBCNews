...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow.
    sfm = softmax(np.dot(outsideVectors, centerWordVec))
    loss = -np.log(sfm[outsideWordIdx])
    outsideVect = outsideVectors[outsideWordIdx]
    gradCenterVec = -(outsideVect - np.matmul(sfm, outsideVectors))
    sfm = np.reshape(sfm, (sfm.shape[0], 1))
    gradOutsideVecs = np.multiply(sfm, centerWordVec)
    gradOutsideVecs[outsideWordIdx] = -centerWordVec + gradOutsideVecs[outsideWordIdx]
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    inds = list(collections.Counter(indices).keys())
    keys = list(collections.Counter(indices).values())
    sig = sigmoid(np.dot(outsideVectors[inds], centerWordVec))
    loss = -np.log(sig[0]) - np.sum(np.multiply(np.log(1-sig[1:]).T, keys[1:]).T, axis=0)
    sig = np.reshape(sig, (sig.shape[0], 1))
    gradCenterVec = -(1-sig[0]) * outsideVectors[outsideWordIdx] - np.sum(np.multiply(np.multiply((-sig[1:]), outsideVectors[inds[1:]]).T, keys[1:]).T, axis=0)
    gradOutsideVecs = np.zeros((len(outsideVectors), centerWordVec.shape[0]))
    gradOutsideVecs[inds[1:]] += np.multiply(np.multiply((sig[1:]), centerWordVec).T, keys[1:]).T
    gradOutsideVecs[outsideWordIdx] -= (1 - sig[0]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    indices = [word2Ind[word] for word in outsideWords]
    centerWordVector = centerWordVectors[word2Ind[currentCenterWord]]
    out = np.array([word2vecLossAndGradient(centerWordVector, ind, outsideVectors, dataset) for ind in indices], dtype=object)
    [Jloss, JgradCenterVecs, JgradOutsideVectors] = [out[:, 0], out[:, 1], out[:, 2]]
    loss = np.sum(Jloss, axis=0)
    gradCenterVecs[word2Ind[currentCenterWord]] = np.sum(JgradCenterVecs,axis=0)
    gradOutsideVectors = np.sum(JgradOutsideVectors, axis=0)
