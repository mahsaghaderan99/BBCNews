...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        # source_padded = source_padded.transpose(0,1)
        # target_padded = target_padded.transpose(0,1)
        enc_hiddens, dec_init_state = self.encode(source_padded, source_lengths)
        enc_masks = self.generate_sent_masks(enc_hiddens, source_lengths)
        combined_outputs = self.decode(enc_hiddens, enc_masks, dec_init_state, target_padded)
        P = F.log_softmax(self.target_vocab_projection(combined_outputs), dim=-1)
        target_masks = (target_padded != self.vocab.tgt['<pad>']).float()
        # target_padded_t = target_padded.transpose(0,1)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        #print(source_padded,'source')
        X = self.model_embeddings.source(source_padded)
        # X = X.transpose(0,1)
        # print('x',X.shape)
        # print('source_lengths',len(source_lengths))
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # print('pad targ',target_padded.shape)
        Y = self.model_embeddings.target(target_padded)
        # print('y',Y.shape)
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t_s = torch.squeeze(Y_t)
            # print('o_t',o_prev.shape)
            # print('ts_t',Y_t_s.shape)
            Ybar_t = torch.cat((Y_t_s, o_prev), dim=1)
            # print('ybar',Ybar_t.shape)
            dec_state, o_t, e_t = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(o_t)
            o_prev = o_t
        combined_outputs = torch.stack(combined_outputs, dim=0)
        # print('combined_outputs', combined_outputs.shape)
        ### END YOUR CODE
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print('dec_hidden_us',dec_hidden_us.shape)
        # print('enc_hiddens_proj',enc_hiddens_proj.shape)
        # print('e_t',e_t.shape)
